...
export const editEmployeeSchema = object({
  ...
  email: string().optional().email('No es un email válido').required('El email es obligatorio'),
  password: string()
    .optional()
  ...
  position: string()
    .optional()
    .transform((value, originalValue) => (originalValue === '' ? null : value))
    .oneOf(['admin', 'seller', 'cashier', 'support', 'other'], 'Tipo de documento no válido'),
  phone: string()
    .optional()
    .transform((value, originalValue) => (originalValue === '' ? null : value))
